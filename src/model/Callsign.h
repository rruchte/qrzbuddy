...
namespace qrz
{
	...
	public:
		...
		/**
		 * @brief Gets the signal-to-noise ratio (SNR).
		 *
		 * This function returns the SNR value as an integer. The SNR represents the
		 * ratio of the signal power to the noise power. A higher SNR value indicates
		 * a stronger signal and less noise.
		 *
		 * @return The SNR value as an integer.
		 *
		 * @see setSnr()
		 */
		int getSnr() const
		...
		/**
		 * @brief Sets the signal-to-noise ratio (SNR) for the calling object.
		 *
		 * This function sets the SNR for the Callsign object. The SNR represents the ratio of the signal power to the noise power, and is often used as a measure of signal quality.
		 *
		 * @param snr The SNR value to be set.
		 * @return void
		 */
		void setSnr(int snr)
		{
		...
		/**
		 * @brief Get the reported signal-to-noise ratio (SNR).
		 *
		 * This function returns the reported SNR value.
		 *
		 * @return The reported SNR.
		 */
		int getReportedSnr() const
		{
			return reportedSnr;
		}

		/**
		 * @brief Set the reported signal-to-noise ratio.
		 *
		 * This function sets the reported signal-to-noise ratio (SNR) for a Callsign object.
		 * The reported SNR is a measure of the quality of the signal received.
		 *
		 * @param reportedSnr The reported SNR value to be set.
		 */
		void setReportedSnr(int reportedSnr)
		{
			Callsign::reportedSnr = reportedSnr;
		}

		/**
		* @brief Retrieves the "last heard" timestamp
		*
		* @return A constant reference to a std::string object representing the value of "last heard" timestamp.
		*/
		const std::string &getLastHeard() const
		{
			return m_lastHeard;
		}

		/**
		 * @brief Sets the value of m_lastHeard.
		 *
		 * This function is used to set the value of m_lastHeard variable.
		 *
		 * @param mLastHeard The new value to set for m_lastHeard.
		 */
		void setLastHeard(const std::string &lastHeard)
		{
			m_lastHeard = lastHeard;
		}
	...
	private:
		...
		// The SNR we hear for this callsign
		int snr = -99;

		// The SNR report for our station sent by this callsign
		int reportedSnr = -99;

		// The "last heard" time based upon JS8Call message data
		std::string m_lastHeard;
	};
}
